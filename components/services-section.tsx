      ...
      <div className="container mx-auto px-4 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services_data?.map((service, index) => (
            ...
            >
              <Card className="h-full transition-all cursor-pointer duration-300 bg-white border-none hover:bg-[#0C708E] group">
                <CardContent className="p-6">
                  <div className="flex items-center justify-center w-16 h-16 bg-gray-100 rounded-lg mb-4 group-hover:bg-white">
                    <service.icon className="w-8 h-8 text-black group-hover:text-[#0C708E]" />
                  </div>
                  <h2 className="text-xl font-semibold text-black mb-2 group-hover:text-white">
                    {service.title}
                  </h2>
                  <div className="w-8 h-0.5 bg-[#0C708E] mb-3 group-hover:bg-white" />
                </CardContent>
              </Card>
            </motion.div>
          ))}
