            ...
            {testimonials.map((testimonial) => (
              <CarouselItem key={testimonial.id}>
                <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
                  <div className="w-16 h-16 md:w-20 md:h-20 bg-[#0C708E] rounded-full flex items-center justify-center shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer group-hover:scale-110">
                  ...
                  <blockquote className="text-2xl md:text-4x font-light leading-relaxed mb-10 px-4">
                    <span className="text-[#0C708E] text-3xl leading-none">&ldquo;</span>
                    <span className="italic text-white/90">
                      {testimonial.text}
                    </span>
                    <span className="text-[#0C708E] text-3xl leading-none"> &rdquo;</span>
                  </blockquote>
                  ...
                  <div className="flex items-center gap-1 mb-8">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        className="w-6 h-6 fill-[#0C708E] stroke-[#0C708E]"
                      />
                    ))}
                  </div>
                </div>
            ...
            >
              <div
                className={`w-3 h-3 rounded-full transition-all duration-300 
                  ${index === current ? "bg-[#0C708E]" : "bg-white/30 hover:bg-white/50"}`}
              />
            </Button>
          ))}
        </div>
